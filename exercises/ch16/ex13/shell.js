    ...
    case ">": // RedirCmd
      {
        ...
        const writeStream = fs.createWriteStream(cmd.file); // ファイルストリームを作成
        await runcmd(cmd.cmd, stdin, writeStream); // 再帰的に runcmd を実行し出力をファイルにリダイレクト
        writeStream.end(); // ストリームを終了
      }
      break;
