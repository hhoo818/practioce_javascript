import { Server } from "http"; // Use "https" if you have a certificate
import { parse } from "url"; // For parsing URLs
import { resolve, extname } from "path"; // For manipulating filesystem paths
import { createReadStream, createWriteStream } from "fs"; // For reading files

// Serve files from the specified root directory via an HTTP server that
// listens on the specified port.
function serve(rootDirectory, port) {
  let server = new Server(); // Create a new HTTP server
  ...
  server.on("request", (request, response) => {
    ...
    // ファイルアップロード
      ...
      const writeStream = createWriteStream(`./output${endpoint}`);
